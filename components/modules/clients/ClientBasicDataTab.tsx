// components/modules/clients/ClientBasicDataTab.tsx
"use client"
...
interface ClientBasicDataTabProps {
  data: CreateClientData
  onChange: (field: string, value: any) => void
  errors: Record<string, string>
}

export default function ClientBasicDataTab({ data, onChange, errors }: ClientBasicDataTabProps) {
  const { generateCode, loading } = useClientOperations()
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={data.tipo_cliente} onValueChange={(value) => onChange('tipo_cliente', value)}>
                <SelectTrigger className={errors.tipo_cliente ? 'border-red-500' : ''}>
              ...
              {errors.tipo_cliente && (
                <p className="text-sm text-red-500">{errors.tipo_cliente}</p>
              )}
            </div>
          </div>
        </CardContent>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="razao_social">
              {data.tipo_cliente === 'fisica' ? 'Nome Completo' : 'Razão Social'} *
            </Label>
            <Input
              id="razao_social"
              value={data.razao_social}
              onChange={(e) => onChange('razao_social', e.target.value)}
              placeholder={data.tipo_cliente === 'fisica' ? 'Nome completo da pessoa' : 'Razão social da empresa'}
          ...
          <div className="space-y-2">
            <Label htmlFor="nome_fantasia">
              {data.tipo_cliente === 'fisica' ? 'Apelido' : 'Nome Fantasia'}
            </Label>
            <Input
              id="nome_fantasia"
              value={data.nome_fantasia || ''}
              onChange={(e) => onChange('nome_fantasia', e.target.value)}
              placeholder={data.tipo_cliente === 'fisica' ? 'Como prefere ser chamado' : 'Nome fantasia da empresa'}
            />
          </div>
        </CardContent>
      </Card>
